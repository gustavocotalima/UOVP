...
import { calculateDiagramScore, DEFAULT_QUESTIONS } from "@/features/portfolio/questions";

describe("diagrama", () => {
  it("soma +1 para sim e -1 para não", () => {
    expect(calculateDiagramScore([true, true, false, true, false])).toBe(1);
  });

  it("mantém o modelo padrão do Diagrama do Cerrado na ordem da AUVP", () => {
    expect(DEFAULT_QUESTIONS.filter(({ type }) => type === "CERRADO")).toEqual([
  ...
  it("mantém o modelo padrão de investimentos imobiliários na ordem da conta AUVP", () => {
    expect(DEFAULT_QUESTIONS.filter(({ type }) => type === "REAL_ESTATE")).toEqual([
