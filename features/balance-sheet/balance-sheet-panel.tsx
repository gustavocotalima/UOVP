        ...
        <CardContent className="space-y-5">
          {BALANCE_CATEGORIES.filter((category) => BALANCE_META[category].side === side).map((category) => (
            <section key={category}>
              ...
              <div className="space-y-2">{entries.filter((entry) => entry.category === category).map((entry) => <div key={entry.id} className="flex items-center gap-2 rounded-xl border p-3 text-sm"><span className="flex-1">{entry.name}</span><strong>{formatMoney(entry.value)}</strong><Button variant="ghost" size="icon" onClick={() => setEditing({ id: entry.id, category, name: entry.name, value: Number(entry.value) })}><Pencil className="size-4" /></Button><Button variant="ghost" size="icon" onClick={() => confirm("Excluir este lançamento?") && startTransition(() => deleteBalanceEntryAction(entry.id))}><Trash2 className="size-4 text-[var(--danger)]" /></Button></div>)}</div>
