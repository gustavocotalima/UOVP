...
export function BudgetClient({ data }: { data: BudgetDto }) {
  ...
  async function removeExpense(expense: BudgetDto["expenses"][number]) {
    const confirmed = await requestConfirmation({
      title: "Excluir gasto?",
      description: `${expense.name} será removido de ${monthLabel}. Esta ação não pode ser desfeita.`,
      confirmLabel: "Excluir gasto",
      danger: true,
    });
    if (confirmed) handleAction(() => deleteExpenseAction(expense.id), "Gasto excluído.");
  }
